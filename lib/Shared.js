...
module.exports = function Shared(context) {
	var share = {
		...
		handleRangeHeaders: function handleRangeHeaders(content, ctx) {
			//assumes express API. For other servers, need to add logic to access alternative header APIs
			ctx.set("Accept-Ranges", "bytes");
			if(ctx.headers.range) {
				var ranges = parseRange(content.length, ctx.headers.range);
		...
		ready: function ready(fn, ctx) {
			var options = context.options;
			if(context.state) return fn(context.webpackStats);
			if(!options.noInfo && !options.quiet)
				options.log("webpack: wait until bundle finished: " + (ctx.url || fn.name));
		...
		handleRequest: function(filename, processRequest, ctx) {
			...
			share.ready(processRequest, ctx);
		},
